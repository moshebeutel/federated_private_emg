...
def federated_train_single_epoch(model, loss_fn, optimizer,
                                 public_users_list,
                                 train_user_list, train_params: TrainParams,
                                 gep=None,
                                 GPs=None,
    ...
    public_users = public_users_list if not Config.SANITY_CHECK else clients_in_epoch
    if Config.USE_GEP:
    ...
    clients_in_epoch = [*public_users, *clients_in_epoch] \
        if (not Config.SANITY_CHECK and Config.USE_GEP) else clients_in_epoch
...
def federated_train_model(model,
                          loss_fn,
                          public_users_list,
                          train_user_list,
    ...
    for epoch in range(num_epochs):
        ...
        if log2wandb:
            log_dict = {'epoch_train_loss': train_loss,
